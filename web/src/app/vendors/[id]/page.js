...
export default function VendorPublicProfile({ params }) {
  ...
  const loadVendorData = async () => {
    try {
      ...
      if (profsData.length > 0) {
        try {
          const scheduleData = await availabilityAPI.getProfessionalSchedule(
            profsData[0].id
          );
  ...
  const handleBookNow = (service) => {
    ...
    if (user.user_type === "vendor" || user.user_type === "professional") {
      alert(
        "Vendors and professionals cannot book services. Please use a customer account."
      );
      return;
    }
                  ...
                  {professionals.map((prof) => (
                    ...
                    >
                      {prof.avatar_url ? (
                        ...
                        <div className="w-16 h-16 rounded-full bg-[#F5F0EB] flex items-center justify-center text-[#8B7355] font-bold text-xl">
                  ...
                  <>
                    ...
                    <div className="w-full h-48 bg-neutral-200 rounded-lg overflow-hidden mb-3">
                      <iframe
                        ...
                        }&q=${encodeURIComponent(vendor.location)}`}
                        allowFullScreen
                      ></iframe>
                    </div>
              ...
              <div className="bg-white rounded-xl border border-neutral-200 p-6">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center gap-2 text-neutral-700">
                    ...
                    <span>
                      {vendor.total_professionals || 1} team member
                      {(vendor.total_professionals || 1) > 1 ? "s" : ""}
                    </span>
                  </div>
                </div>
              </div>
